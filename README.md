...
Полнофункциональное веб-приложение для планирования циклических тренировочных программ, ежедневного трекинга выполнения заданий, аналитики мышечной нагрузки по 13 группам мышц и обмена тренировками с друзьями в формате социальной сети. Интерфейс выполнен в стиле **Instagram**: фиксированная панель навигации, карточки контента, подписки, лента и уникальная кнопка **«IN»** — лайк, который одновременно клонирует тренировку в вашу коллекцию.
...
┌──────────────────────────────────────────────────┐
│                   Браузер (SPA)                  │
│  React 18 + React Router 6 + Vite + Lucide Icons │
│  Авторизация: localStorage (username + password)  │
└──────────────────┬───────────────────────────────┘
                   │ /api/* (Vite proxy → :8001)
┌──────────────────▼───────────────────────────────┐
│              FastAPI-сервер (:8001)               │
│  Pydantic-валидация, bcrypt-аутентификация, CORS  │
│  Аналитический движок мышечной нагрузки           │
│  30+ REST API эндпоинтов                          │
└──────────────────┬───────────────────────────────┘
                   │ SQLAlchemy ORM
┌──────────────────▼───────────────────────────────┐
│            SQLite (db/db.db)                      │
│  Таблицы: users, cycles, notes, follows,          │
│           likes, comments                         │
│  + exercises.json (каталог 22 упражнений)         │
└──────────────────────────────────────────────────┘
```
